...
for data_i in all_datasets:
    ...
    # backtest settings
    PB.settings['start_date'] = '20060101'
    ...
    PB.settings['results_plot_path'] = os.path.join(RESULTS_PATH,
                                               backtest_combination, 'plots')
    PB.settings['results_tex_path'] = os.path.join(RESULTS_PATH,
                                               backtest_combination, 'tex')
    PB.settings['results_data_path'] = os.path.join(RESULTS_PATH,
                                               backtest_combination, 'data')
...
for measure_i in all_measures:
    ...
    multiindex = pd.MultiIndex.from_tuples(tuples, names=['Vendor', 'Dataset', 'Asset Universe'])
    ...
    caption_text = (f'Out-of-sample mean results {"(in percent)"*(measure_i != "Sharpe")} for the '
        ...
        + f'sample is {data_dates[0]} to {data_dates[1]}.\n\\vspace{{0.6em}}'
        )
    # Create note text for table
    note_text = (f'\\vspace{{0.1em}}\n\n{{\\footnotesize \\textit{{Note:}} '
    ...
    df_highlighted_list.insert(1, '\\fontsize{9}{14}\\selectfont{')
    
    for i in [1, 3]:
    
        tmp_index = df_highlighted_list.index('\\toprule')+i    
        tmp_string = df_highlighted_list[tmp_index]
        multicol_len = [i+1 for i in range(len(tmp_string)) if tmp_string.endswith('multicolumn{', 0, i + 1)]
    ...
    tex_file_name = f"{all_datasets[data_i]}_{backtest_combination}_{measure_i.replace(" ", "")}.tex"
    with open(os.path.join(PB.settings['results_tex_path'], tex_file_name), "w") as f:
