...
import cvxpy
from aggregated_functions import annual_measures, turnover_measures
...
class PortfolioBacktest(object):
    ...
    def get_data(self):
        ...
        # read data
        data = pd.read_excel(self.settings['data_path']+self.settings['data_set']+'.xlsx')
        data['Date'] = pd.to_datetime(data['Date'], format='%Y%m')
        data['Date'] = data['Date'].apply(last_day_of_month)
        data.set_index('Date', inplace=True)
        data.index.name = None
        # clean data
        data[data<=-99.99] = 0 # cut NaNs
        data = data/100 # decimal returns
        # save data
        self.data['returns'] = data
        self.data['index'] = pd.DataFrame(data.mean(axis=1),columns=['Index'])
        self.data['dates'] = data.index
        return
    def calc_allocation(self, returns):
        ...
        returns = returns.loc[:, (returns.sum(axis=0) !=0)]
        
        if self.settings['opt_method'] == 1: # GMVP (no constraints)
            
            y = np.array(returns.iloc[:,0])
            x = np.array(-returns.iloc[:,1:].add(y, axis=0))
            x = np.insert(x,0,1,axis=1)
            weights = np.linalg.pinv(x.T.dot(x)).dot(x.T.dot(y))
            final_weights = np.append(1-weights.sum(),weights[1:]).round(8)
            
        elif self.settings['opt_method'] == 12: # GMVP (as regression)
            
            N = returns.shape[1]
            y = np.array(returns.mean(axis=1))
            x = np.array(-returns.add(y, axis=0))
            x = np.insert(x,0,1,axis=1)
            weights_tilde = np.linalg.pinv(x.T.dot(x)).dot(x.T.dot(y))
            final_weights = weights_tilde[1:] + (1/N) * (1 - weights_tilde[1:].sum())
            final_weights = final_weights.round(8)
            
        elif self.settings['opt_method'] == 2: # GMVP (no short sales)
        ...
        elif self.settings['opt_method'] == 3: # 1/N portfolio
            
            final_weights = np.ones((returns.shape[1], 1))/returns.shape[1]
        
        elif self.settings['opt_method'] == 4: # 1/vol portfolio
            
            final_weights = 1/np.std(returns)
            final_weights = final_weights / final_weights.sum()
        
        elif self.settings['opt_method'] == 5: # equal risk contribution portfolio
    ...
    def backtest_allocation(self):
        ...
        for i in range(0, len(dates_reb), 1):
            ...
            hist_alloc_tmp = self.calc_allocation(tmp_returns)
    ...
    def complete_allocation(self):
        ...
        allocations, dates = self.backtest_allocation()
    ...
    def backtest(self):        
        ...
        allocation, dates = self.complete_allocation()
    ...
    def plot_backtest(self, backtest_output, *years):
        ...
        # Equity Lines
        fig = plt.figure()
        plt.plot(((1+portfolio_returns).cumprod()*100))
        plt.legend(['Portfolio', 'Index'], frameon=False)
        ...
        each_year = False
        if each_year:
            for i in enumerate(portfolio_returns.index.year.unique()):
                fig = plt.figure()
                try:
                    first_obs = pd.DataFrame([0,0], index=portfolio_returns.columns).T#, columns=portfolio_returns[str(i[1])].index[0]-dt.timedelta(days=1))#pd.DataFrame(columns=portfolio_returns.columns) pd.DataFrame(0) -dt.timedelta(days=30)
                    tmp_portfolio_returns = pd.concat([first_obs, portfolio_returns[str(i[1])]],axis=0)
                    tmp_portfolio_returns.index[0] = pd.Timestamp(str(i[1])+'-01-01')
                except:
                    tmp_portfolio_returns = portfolio_returns[str(i[1])]
                    pass
        ...
        fig = plt.figure()
        window = self.settings['window']
        plt.plot(portfolio_returns.rolling(window=window,min_periods=1).std()*100*np.sqrt(self.settings['length_year']))                 
        ...
        plt.gcf().savefig(self.settings['results_path']+'\\AnnualizedVola_' + years + '_' + years_end + plot_style_type,\
              dpi=200, bbox_inches='tight', pad_inches=0, transparent=False)
        # Turnover                
        turnover_portfolio = backtest_output['daily_turnover']                                                               
        fig = plt.figure()            
